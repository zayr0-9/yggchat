 ...
 */
async function resolveToWindowsPath(filePath, rootPathContext) {
    if (!isWindows())
        return filePath;
 ...
 */
async function normalizeToolPath(p, rootPathContext) {
    if (!p)
        return undefined;
    return resolveToWindowsPath(p, rootPathContext);
}
// Initialize database at specified path
function initializeLocalDatabase(dbPath) {
...
function ensureUserExists(userId) {
    ...
    if (!existing) {
        ...
        db.prepare('INSERT INTO users (id, username, created_at) VALUES (?, ?, ?)').run(userId, `synced-user-${userId.substring(0, 8)}`, new Date().toISOString());
    }
}
function ensureProjectExists(projectId, userId) {
    if (!db)
        return;
    ...
    if (!existing) {
        ...
        db.prepare('INSERT INTO projects (id, name, user_id, context, system_prompt, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run(projectId, 'Synced Project', userId, null, null, now, now);
    }
}
function ensureConversationExists(conversationId, userId, projectId) {
    if (!db)
        return;
    ...
    if (!existing) {
        ...
        const now = new Date().toISOString();
        db.prepare('INSERT INTO conversations (id, project_id, user_id, title, model_name, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run(conversationId, projectId || null, userId, 'Synced Conversation', 'unknown', now, now);
    }
}
            ...
            try {
                ...
                if (client.type === 'frontend') {
                    if (message.type === 'request_context') {
                        const extensionClients = Array.from(clients).filter(c => c.type === 'extension' && c.ws.readyState === WebSocket.OPEN);
                        clients.forEach(c => {
...
function setupServer() {
    ...
    app.post('/api/sync/project', (req, res) => {
        try {
            ...
            statements.upsertProject.run(id, name, effectiveUserId, context || null, system_prompt || null, created_at || new Date().toISOString(), updated_at || new Date().toISOString());
    ...
    app.post('/api/sync/conversation', (req, res) => {
        try {
            const { id, project_id, user_id, owner_id, // Railway uses owner_id, local uses user_id
            title, model_name, system_prompt, conversation_context, research_note, cwd, created_at, updated_at, } = req.body;
            // Handle owner_id -> user_id mapping (Railway sends owner_id)
            const effectiveUserId = user_id || owner_id;
            if (!effectiveUserId) {
                res.status(400).json({ error: 'Missing user_id or owner_id' });
            ...
            statements.upsertConversation.run(id, project_id || null, effectiveUserId, title || null, model_name || 'unknown', system_prompt || null, conversation_context || null, research_note || null, cwd || null, created_at || new Date().toISOString(), updated_at || new Date().toISOString());
    ...
    app.post('/api/sync/attachment', (req, res) => {
        try {
            const { id, message_id, kind, mime_type, storage, url, file_path, width, height, size_bytes, sha256, created_at, } = req.body;
            statements.upsertAttachment.run(id, message_id || null, kind, mime_type, storage || 'url', url || null, file_path || null, width || null, height || null, size_bytes || null, sha256 || null, created_at || new Date().toISOString());
    ...
    app.post('/api/sync/provider-cost', (req, res) => {
        try {
            const { id, user_id, message_id, prompt_tokens, completion_tokens, reasoning_tokens, approx_cost, api_credit_cost, created_at, } = req.body;
            statements.upsertProviderCost.run(id, user_id, message_id, prompt_tokens || 0, completion_tokens || 0, reasoning_tokens || 0, approx_cost || 0, api_credit_cost || 0, created_at || new Date().toISOString());
            ...
            for (const op of operations) {
                try {
                    switch (op.type) {
                        ...
                        case 'project':
                            if (op.action === 'create' || op.action === 'update') {
                                statements.upsertProject.run(op.data.id, op.data.name, op.data.user_id, op.data.context || null, op.data.system_prompt || null, op.data.created_at || new Date().toISOString(), op.data.updated_at || new Date().toISOString());
                        ...
                        case 'conversation':
                            if (op.action === 'create' || op.action === 'update') {
                                statements.upsertConversation.run(op.data.id, op.data.project_id || null, op.data.user_id, op.data.title || null, op.data.model_name || 'unknown', op.data.system_prompt || null, op.data.conversation_context || null, op.data.research_note || null, op.data.cwd || null, op.data.created_at || new Date().toISOString(), op.data.updated_at || new Date().toISOString());
                        ...
                        case 'message':
                            if (op.action === 'create' || op.action === 'update') {
                                statements.upsertMessage.run(op.data.id, op.data.conversation_id, op.data.parent_id || null, typeof op.data.children_ids === 'string'
    ...
    app.post('/api/tools/execute', async (req, res) => {
        try {
            ...
            const resolvePath = (p) => normalizeToolPath(p, rootPath);
    ...
    app.get('/api/sync/stats', (_req, res) => {
        try {
            const stats = {
                ...
                attachments: db.prepare('SELECT COUNT(*) as count FROM message_attachments').get(),
            };
            res.json(stats);
        }
        catch (error) {
...
// Start the server
export function startLocalServer(port = 3002, dbPath) {
