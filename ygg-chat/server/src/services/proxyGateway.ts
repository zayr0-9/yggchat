...
let credentialModulePromise:
  | Promise<typeof import('../database/credential_table')>
  | null = null

const loadCredentialModule = async () => {
  if (!credentialModulePromise) {
    credentialModulePromise = import('../database/credential_table')
  }
...
class ProxyGateway {
  ...
  private createCredentialVault(options?: ProxyGatewayOptions): CredentialVault {
    ...
    // Auto-detect: prefer Supabase when available (web mode), fall back to file
    const source = explicitSource ?? (hasSupabaseConfig() ? 'supabase' : DEFAULT_CREDENTIALS_SOURCE)
    ...
    if (source === 'supabase') {
      logProxyEvent('info', 'using_supabase_credential_vault', {})
