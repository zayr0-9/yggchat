...
export class CreditsService {
  ...
  static async decrementCredits(userId: string, amount: number, reason: string): Promise<number | null> {
    try {
      ...
      if (!check.hasCredits) {
        console.warn(`[CreditsService] Insufficient credits for user ${userId}. Required: ${amount}, Available: ${check.currentBalance}`)
        return null
      }
...
export class SubscriptionService {
  ...
  static async upsertSubscription(params: {
    ...
    try {
      ...
      console.log(`[SubscriptionService] Upserted subscription ${params.stripeSubscriptionId} for user ${params.userId}`)
