...
import { SubscriptionService, UserService, supabaseAdmin, CreditsService } from '../database/supamodels'
  ...
  try {
    ...
    try {
      ...
      if (invoiceId) {
        ...
        console.log(
          `[Stripe] Initial credits allocated for user ${userId}, invoice: ${invoiceId}`
        )
      } else {
        console.warn(
          `[Stripe] No invoice ID in checkout session ${session.id}. Credits may not be allocated.`
        )
      }
    } catch (creditError) {
...
export async function handleInvoicePaymentFailed(invoice: Stripe.Invoice): Promise<void> {
  ...
  try {
    ...
    if (!userId) {
      throw new Error(`Missing userId in subscription metadata. Subscription: ${subscriptionId}, Invoice: ${invoice.id}`)
    }
...
export async function handleSubscriptionCreated(subscription: Stripe.Subscription): Promise<void> {
  ...
  try {
    ...
    console.log(`[Stripe] Subscription created for user ${userId}, subscription: ${subscription.id}, status: ${subscription.status}`)
  } catch (error) {
...
export async function handleSubscriptionUpdated(subscription: Stripe.Subscription): Promise<void> {
  ...
  try {
    ...
    console.log(`[Stripe] Subscription updated for user ${userId}, subscription: ${subscription.id}, status: ${subscription.status}`)
  } catch (error) {
...
export async function handleSubscriptionDeleted(subscription: Stripe.Subscription): Promise<void> {
  try {
    ...
    console.log(`[Stripe] Subscription canceled for user ${userId}, subscription: ${sub.id}, access until ${periodEndDate}`)
  } catch (error) {
