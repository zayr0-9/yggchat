...
function findMatchWithStrategies(
  ...
  if (lineEndingIndex !== -1) {
    ...
    const actualStartIndex = findActualPosition(content, normalizedContent, lineEndingIndex)
 ...
 */
function findActualPosition(original: string, normalized: string, normalizedIndex: number): number {
